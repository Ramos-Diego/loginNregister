...
exports.loginUser = (req, res, next) => {
  ...
  // TODO: how to share among middleware
  const redirectAndFlash = (path, message) => {
    req.session.flash = {
      type: 'error',
      email: email,
      message: message
  ...
  User.findOne({ email: email })
    .then((user) => {
      ...
      if (!user) { redirectAndFlash('/login', 'Email or password incorrect.') }
      ...
      if (isValid) {
        ...
        return res.redirect('/')
      } else { redirectAndFlash('/login', 'Email or password incorrect.') }
    })
...
exports.registerUser = async (req, res, next) => {
  ...
  try {
    ...
    if (!emailExists) {
      ...
      console.log('User account succesfully saved!', saved)
